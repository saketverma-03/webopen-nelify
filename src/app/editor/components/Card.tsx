...
export default function CardComponent({
  ...
  const mutatePublic = useMutation({
    mutationFn: async (data: { isChecked: boolean }) => {
      ...
      console.log(data);
      const res = await axios.put(`/api/project/${id}/changepublic`, {
        isPublic: isChecked,
      });
      return res.data;
    },
    ...
    <>
      <Card className="w-full hover:border-primary">
        <CardHeader className="flex flex-row">
          ...
          <span>
            {mutatePublic.isLoading ? (
              <Loader2Icon className="animate-spin" />
            ) : (
              isChecked && (
                <Share2
                  onClick={() => {
                    navigator.clipboard.writeText(`${pathname}/editor/${id}`);
                  }}
                />
              )
            )}
